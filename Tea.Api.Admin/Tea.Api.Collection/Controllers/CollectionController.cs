...
namespace Tea.Api.Collection.Controllers
{
    ...
    public class CollectionController : ControllerBase
    {
        ...
        [HttpPost, Route("GetStgRateFixModifyData")]
        public async Task<IActionResult> GetStgRateFixModifyData([FromBody] GetStgRateFixModel _input)
        {
            var results = await _collectionService.GetStgRateFixModifyData(_input);
            string JsonResult;
            JsonResult = JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented);
            return (results != null) ? Ok(JsonResult) : throw new Exception();
        }
        ...
        [HttpPost, Route("GetSupplierMobileData")]
        public async Task<IActionResult> GetSupplierMobileData([FromBody] GetSupplierMobileModel _input)
        {
            var results = await _collectionService.GetSupplierMobileData(_input);
            string JsonResult;
            JsonResult = JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented);
            return (results != null) ? Ok(JsonResult) : throw new Exception();
        }
        ...
        [HttpPost, Route("GetLockedVehicleList")]
        public async Task<IActionResult> GetLockedVehicleList([FromBody] LockVehicleListModel _input)
        {
            var results = await _collectionService.GetLockedVehicleList(_input);
            string JsonResult;
            JsonResult = JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented);
            return (results != null) ? Ok(JsonResult) : throw new Exception();
        }
        ...
        [HttpPost, Route("GetSupplierHistoryFactory")]
        public async Task<IActionResult> GetSupplierHistoryFactory([FromBody] GetSaleRateFixFactory _input)
        {
            var results = await _collectionService.GetSupplierHistoryFactory(_input);
            string JsonResult;
            JsonResult = JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented);
            return (results != null) ? Ok(JsonResult) : throw new Exception();
        }

        [HttpPost, Route("GetSupplierHistory")]
        public async Task<IActionResult> GetSupplierHistory([FromBody] ReportHistoryFilterModel _input)
        {
            var results = await _collectionService.GetSupplierHistory(_input);
            string JsonResult;
            JsonResult = JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented);
            return (results != null) ? Ok(JsonResult) : throw new Exception();
        }

        [HttpPost, Route("GetStgRateFixGradeData")]
        public async Task<IActionResult> GetStgRateFixGradeData([FromBody] GetStgRateFixFilterModel _input)
        {
            var results = await _collectionService.GetStgRateFixGradeData(_input);
            string JsonResult;
            JsonResult = JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented);
            return (results != null) ? Ok(JsonResult) : throw new Exception();
        }
