...
namespace Tea.Api.Collection.Controllers
{
    ...
    public class ReportsController : ControllerBase
    {
        ...
        [HttpPost, Route("SalePurchaseWiseReport")]
        public async Task<IActionResult> SalePurchaseWiseReport([FromBody] GradeReportModel _input)
        {
            var results = await _collectionService.SalePurchaseWiseReport(_input);
            string JsonResult;
            JsonResult = JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented);
            return (results != null) ? Ok(JsonResult) : throw new Exception();
        }

        [HttpPost, Route("GetSeasonAdvanceReport")]
        public async Task<IActionResult> GetSeasonAdvanceReport([FromBody] SeasonAdvReportModel _input)
        {
            var results = await _collectionService.GetSeasonAdvanceReport(_input);
            string JsonResult;
            JsonResult = JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented);
            return (results != null) ? Ok(JsonResult) : throw new Exception();
        }
        ...
        [HttpPost, Route("GetFieldBalanceReport")]
        public async Task<IActionResult> GetFieldBalanceReport([FromBody] SeasonAdvReportModel _input)
        {
            var results = await _collectionService.GetFieldBalanceReport(_input);
            string JsonResult;
            JsonResult = JsonConvert.SerializeObject(results, Newtonsoft.Json.Formatting.Indented);
            return (results != null) ? Ok(JsonResult) : throw new Exception();
        }
